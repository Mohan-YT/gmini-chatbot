        ...
        {chat.role === "model" ? (
          !chat.hideInChat && (// if hideInChat false then it is work
            <>
                ...
                <p className={`message-text ${chat.isError ? "text-red-500" : "text-black"} ${chat.text && 'text-black'} max-w-[75%] break-words whitespace-pre-line bg-violet-100 rounded-t-xl rounded-br-xl px-2.5  py-2 z-20 `}>{chat.text}</p>
            </>
          )
          )  : (
